...
data_file <- '../results/logs/validation_mobidb_D_CNN_0_l8.txt'
data <- read.table(data_file, sep = '\t', header = TRUE)
...
#  geom_line(data = data[data$Fold==4,], size = 1)+
  ...
  xlim(0,100)+
  ylim(0,100)+
  ggtitle('ROC Curves on Validation Set - Model mobidb_D_CNN_0_l8')+
  xlab('FPR')+
  ylab('TPR')
...
ggplot(mapping = aes(x = FP/(FP+TN)*100, y = Rec, linetype = 'whole dataset,\npositives'))+
  ...
  # aesthetics
  geom_abline(slope = 1, intercept = 0)+
  ggtitle(paste('ROC Curves on Validation Set - Model mobidb_D_CNN_0_l8 Fold', toString(best_fold)))+
  xlab('FPR')+
  ylab('TPR')
