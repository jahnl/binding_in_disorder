...
def FNN_trainer(train_embeddings: str, dataset_dir: str, model_name: str = '2-2_dropout_0.3_lr_0.01', n_splits: int = 5,
    ...
    for fold in range(n_splits):
        ...
        for epoch in range(max_epochs):
            ...
            train(training_dataset, model, criterion, optimizer, device, output_file, batch_size)
            test_loss = test_performance(validation_dataset, model, criterion, device, output_file, batch_size)
        ...
        torch.save(best_state_dict,
                   f"../results/models/binding_regions_model_{model_name}_fold_{fold}.pth")
        output_file.flush()
        output_file.close()
