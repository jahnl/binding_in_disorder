...
def predictCNN(embeddings, cutoff, fold):
...
def predictFNN(embeddings, cutoff, cutoff_p, cutoff_n, cutoff_o, fold, mode, multilabel, post_processing, test):
    output_name = f"../results/logs/predict_val_2-2_dropout_{dropout}_new_{fold}_{cutoff}.txt" if not test else \
        f"../results/logs/predict_val_2-2_dropout_{dropout}_post-process_{cutoff}_test.txt"
...
if __name__ == '__main__':
    ...
    # predictCNN(embeddings, cutoff, fold, mode)
    predictFNN(embeddings, cutoff, cutoff_p, cutoff_n, cutoff_o, fold, mode, multilabel, post_processing, test)
